...
do
  ...
  do
    echo "computing stats for eccentricities between ${MinDegree[$i]} degrees and ${MaxDegree[$i]} degrees"
    ...
    do
      ...
      [ ! -f ./${outname}.txt ] && mri_segstats --annot ./output/ ${h} ./${ecc_var_filename}.annot --ctab ./label.txt --i ./cortexmap/func/${h}.${measures[$j]}.func.gii --surf white --excludeid 0 --o ./${outname}.txt
    ...
    echo "finished computing stats for eccentricities between ${MinDegree[$i]} degrees and ${MaxDegree[$i]} degrees"
  done
done

echo "Finished computing stats! Now onto compilation!"
